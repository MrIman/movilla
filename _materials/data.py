...
PROJECTS = [
    dict(slug="archouse-70", name="ArcHouse", num="70", family="Dach łukowy", family_key="arc", area=69.5,
        tagline="Kompaktowy dom całoroczny z dachem łukowym. Parter z antresolą, przeszklony szczyt i standard deweloperski podwyższony.",
        card_desc="Łukowy dach, antresola i przeszklony szczyt. Dom dla pary lub małej rodziny.", hero="arc70/hero.webp",
        dims="7,28 × 7,60 m", layout="parter + antresola", height="6,9 m", people="",
        params=[("69,50", "m²", "powierzchnia całkowita"), ("7,28 × 7,60", "m", "wymiary w osiach konstrukcyjnych"), ("6,9", "m", "wysokość kalenicy"), ("30–60", "dni", "czas realizacji")],
        ...
        stats=[("Powierzchnia całkowita", "69.5", 1, "m²", "parter i antresola"), ("Parter", "54.12", 2, "m²", "strefa dzienna, kuchnia, łazienka, wiatrołap"), ("Antresola", "15.38", 2, "m²", "na ok. 2/3 długości budynku"),
    ...
    dict(slug="mysa-120", name="MYSA", num="120", family="Dach łukowy", family_key="arc", area=120.2,
        tagline="Większy z domów z dachem łukowym: przestronny parter, pełne piętro lub antresola i ponad 120 m² dla całej rodziny.",
        card_desc="Większy dom łukowy: 90 m² parteru i pełne piętro dla całej rodziny.", hero="mysa120/hero.webp",
        dims="7,28 × 12,90 m", layout="parter + piętro", height="6,9 m", people="",
        params=[("120,20", "m²", "powierzchnia całkowita"), ("7,28 × 12,90", "m", "wymiary w osiach konstrukcyjnych"), ("6,9", "m", "wysokość kalenicy"), ("30–60", "dni", "czas realizacji")],
        ...
        stats=[("Powierzchnia całkowita", "120.2", 1, "m²", "parter i piętro"), ("Parter", "90.33", 2, "m²", "strefa dzienna, sypialnia, łazienka, wiatrołap"), ("Piętro", "29.87", 2, "m²", "antresola lub pełny strop"),
               ("Wysokość", "6.9", 1, "m", "od terenu do kalenicy"), ("U dachu i ścian", "0.15", 2, "W/m²K", "główne przegrody zewnętrzne"), ("Realizacja", "60", 0, "dni", "orientacyjnie 30–60 dni")],
    ...
    dict(slug="barnhouse-85", name="BarnHouse", num="85", family="Dach dwuspadowy", family_key="barn", area=85,
        tagline="Klasyczna forma domu z dachem dwuspadowym w technologii stalowej. 85 m² z antresolą, przeszklonym szczytem i dachem z blachy na rąbek.",
        card_desc="Zwarta bryła z dachem dwuspadowym, przeszklonym szczytem i antresolą.", hero="barn85/hero.webp",
        dims="7,28 × 7,60 m", layout="parter + antresola", height="7,9 m", people="",
        params=[("85", "m²", "powierzchnia całkowita"), ("7,28 × 7,60", "m", "wymiary w osiach konstrukcyjnych"), ("7,9", "m", "wysokość kalenicy"), ("30–60", "dni", "czas realizacji")],
        intro_title="Klasyczna forma. Nowa technologia.",
        intro=["BarnHouse łączy technologię MoVilla z klasyczną formą domu z dachem dwuspadowym: przeszklona ściana szczytowa, dach z blachy na rąbek i prosta, elegancka bryła na rzucie prostokąta.",
        ...
        stats=[("Powierzchnia całkowita", "85", 0, "m²", "parter i antresola"), ("Szerokość", "7.28", 2, "m", "w osiach konstrukcyjnych"), ("Długość", "7.6", 1, "m", "w osiach konstrukcyjnych"),
    ...
    dict(slug="barnhouse-130", name="BarnHouse", num="130", family="Dach dwuspadowy", family_key="barn", area=130,
        tagline="Największy dom w ofercie: 130 m² pod dachem dwuspadowym. Pełna strefa dzienna na parterze i prywatna część nocna na piętrze.",
        card_desc="130 m² dla dużej rodziny: pełne piętro, przeszklony szczyt i taras.", hero="barn130/hero.webp",
        dims="szer. 7,28 m", layout="parter + piętro", height="7,9 m", people="",
        params=[("130", "m²", "powierzchnia całkowita"), ("7,28", "m", "szerokość w osiach konstrukcyjnych"), ("7,9", "m", "wysokość kalenicy"), ("30–60", "dni", "czas realizacji")],
        intro_title="Dom dla całej rodziny. Bez kompromisów.",
        intro=["BarnHouse 130 to rozwinięcie modelu 85: wydłużona bryła z dachem dwuspadowym, która daje pełną strefę dzienną na parterze i wydzieloną część nocną na piętrze.",
        ...
        stats=[("Powierzchnia całkowita", "130", 0, "m²", "parter i piętro"), ("Szerokość", "7.28", 2, "m", "w osiach konstrukcyjnych"), ("Kondygnacje", "2", 0, "", "parter + pełne piętro"),
               ("Wysokość", "7.9", 1, "m", "od terenu do kalenicy"), ("U dachu i ścian", "0.15", 2, "W/m²K", "główne przegrody zewnętrzne"), ("Realizacja", "60", 0, "dni", "orientacyjnie 30–60 dni")],
    ...
    dict(slug="foresthouse", name="ForestHouse", num="", family="A‑frame", family_key="forest", area=90,
        tagline="Kompaktowy dom A‑frame dla 4 osób: całoroczny, rekreacyjny albo na wynajem. W standardzie z wykończeniem wnętrza i wyposażeniem łazienek.",
        card_desc="Dom A‑frame z tarasem. Sprawdzi się też jako obiekt na wynajem.", hero="forest/hero.webp",
        dims="8,30 × 8,00 m", layout="2 sypialnie · 2 łazienki", height="7,0 m", people="4 osoby",
        params=[("90", "m²", "powierzchnia budynku"), ("8,30 × 8,00", "m", "wymiary zewnętrzne"), ("7,0", "m", "wysokość"), ("4", "osoby", "liczba mieszkańców")],
        ...
        variants=[("Dom całoroczny", "Prywatny dom na działce: izolowane przegrody, ogrzewanie i pełne wykończenie wnętrza.", "zastosowanie", "../img/forest/ext-2.webp", "Dom całoroczny"),
                  ("Inwestycja pod wynajem", "Dom gotowy do wynajmu: układ dla 4 osób, dwie łazienki i taras.", "zastosowanie", "../img/forest/ext-3.webp", "Wynajem krótkoterminowy"),
